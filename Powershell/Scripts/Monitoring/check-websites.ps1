...
$File="C:\Temp\Script\Check-Websites\Check-Websites\Logs\$reportname"

If ($alert) {
    
    $Outlook = New-Object -ComObject Outlook.Application    
    $Mail = $Outlook.CreateItem(0)
    $Mail.To = "jan.vrba@neoris.com"
    $Mail.CC = "bqjlocfgxrr+janvrba_test@in.mailroom.hipch.at"
    $Mail.BCC = "jan.vrba@gmx.co.uk"
    $Mail.Subject = "Alert Status changed : Websites Availibility Report for $subjectTime"
    $Mail.HTMLBody = [System.IO.File]::ReadAllText($websiteReport)
...
# Invoke-Expression C:\Temp\Script\Check-Websites\Check-Websites\Logs\$reportname 